...
sygeforApp.config(["$listStateProvider", "$dialogProvider", "$widgetProvider", function($listStateProvider, $dialogProvider, $widgetProvider) {
    ...
    $listStateProvider.state('session', {
        ...
        resolve: {
            training: function($stateParams, $entityManager) {
                if($stateParams.training) {
                    return $entityManager('SygeforCoreBundle:AbstractTraining').find($stateParams.training);
                }
                return null;
            },
        ...
        breadcrumb: function(training, $utils) {
            var breadcrumb = [{ label: "Événements", sref: "training.table" }];
            if(training) {
                breadcrumb.push({ label: $utils.getType(training.type).label, sref: "training.table({type: '" + training.type + "'})" });
    ...
    $dialogProvider.dialog('session.duplicate', /* @ngInject */ {
        controller: function($scope, $modalInstance, $dialogParams, $state, $utils, $http, form, growl) {
            $scope.dialog = $modalInstance;
            $scope.dialog.params = $dialogParams;
            $scope.form = form;
            $scope.$moment = moment;
    ...
    $dialogProvider.dialog('session.evaluationSynthesis', /* @ngInject */ {
        controller: function($scope, $modalInstance, $dialogParams, data, $window) {
            $scope.dialog = $modalInstance;
            $scope.dialog.params = $dialogParams;
            $scope.evaluations = data.evaluations;
            $scope.nbrEvaluation = data.nbrEvaluation;
            $scope.nbrPresentInscription = data.nbrPresentInscription;

            $scope.isType = function(type, field) {
              return typeof field === type;
            };

            $scope.getEvaluationSheet = function() {
                var url = Routing.generate('session.evaluations', {id: $scope.dialog.params.session.id});
                $window.location = url;
            };
        },
        resolve:{
            data: function ($http, $dialogParams){
                return $http.get(Routing.generate('session.evaluation', {id: $dialogParams.session.id })).then(function(response) {
                    return response.data;
                });
            }
        },
        templateUrl: 'training/session/dialogs/evaluation.html'
    });
    ...
    $widgetProvider.widget("session", /* @ngInject */ {
        ...
        options: function($user) {
            return {
                ...
                rights: ['sygefor_core.access_right.training.own.view', 'sygefor_core.access_right.training.all.view'],
                state: 'session.table',
