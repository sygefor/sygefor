...
namespace Sygefor\Bundle\ElasticaUpdateBundle\MappingProvider;
 ...
 * @package Sygefor\Bundle\ElasticaUpdateBundle\CascadeUpdater
...
class MappingProvider
{
    ...
    public function findLinkedEntities($entityClass, $entityProperty, $entityId, $onlyManyToMany = false, $path=array())
    {
        ...
        foreach($metadata->associationMappings as $fieldName => $fieldMD) {
            ...
            if( !empty($fieldMD['targetEntity']) && !in_array($fieldMD['targetEntity'], $this->visitedEntities)) {
                ...
                if (!empty ($invPath)) {
                    ...
                    if ( $this->classMappingContainsPath($fieldMD['targetEntity'], join(".", $tmpArr)) ) {
