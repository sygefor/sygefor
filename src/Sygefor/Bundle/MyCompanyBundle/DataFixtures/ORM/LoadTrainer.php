...
class LoadTrainer extends AbstractTestDataFixture
{
    ...
    protected function doLoad(ObjectManager $manager)
    {
        ...
        foreach ($organizations as $organization) {
            $populator->addEntity('Sygefor\Bundle\MyCompanyBundle\Entity\Trainer', 1, array(
                ...
                'trainerType' => function () use ($manager, $organization) {
                    $trainerTypes = $manager->getRepository(TrainerType::class)->findAll();
                    $size = count($trainerTypes);
                    if ($size === 0) {
                        return null;
                    }

                    return $trainerTypes[rand(0, $size - 1)];
                },
                'service' => function () use ($faker) {
                    return $faker->sentence($nbWords = 3);
                },
                'status' => function () use ($faker) {
                    return $faker->sentence($nbWords = 7);
                },
                'responsabilities' => function () use ($faker) {
                    return $faker->sentence($nbWords = 7);
                },
                'isArchived' => function () use ($faker) {
                    return $faker->boolean($chanceOfGettingTrue = 50);
                },
                'isAllowSendMail' => function () use ($faker) {
                    return $faker->boolean($chanceOfGettingTrue = 50);
                },
                'isOrganization' => function () use ($faker) {
                    return $faker->boolean($chanceOfGettingTrue = 50);
                },
                'isPublic' => function () use ($faker) {
                    return $faker->boolean($chanceOfGettingTrue = 50);
                },
                'observations' => function () use ($faker) {
                    return $faker->sentence($nbWords = 7);
                },
            ));
            $populator->execute();
