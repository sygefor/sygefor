 ...
 * @package Sygefor\Bundle\TrainingBundle\SpreadSheet
 */
class TrainingBalanceSheet
{
    /**
    ...
    private function collectSessionInfos ()
    {
        ...
        foreach ($sessions as $session) {
            $presentRegistrations = $this->container->get('doctrine')->getRepository('SygeforInscriptionBundle:AbstractInscription')->findBy(array(
                'session' => $session,
                'presenceStatus' => $this->container->get('doctrine')->getRepository('SygeforInscriptionBundle:Term\PresenceStatus')->findBy(array(
                    'machineName' => 'present'
                ))
            ));
        ...
        $publicType = $this->container->get('doctrine')->getRepository('SygeforTraineeBundle:Term\PublicType')->findAll();
    ...
    protected function buildBalanceSheet()
    {
        ...
        $currentRow++;
        //theme
        $activeSheet->setCellValue('A'.$currentRow, "Domaine de connaissance :")
