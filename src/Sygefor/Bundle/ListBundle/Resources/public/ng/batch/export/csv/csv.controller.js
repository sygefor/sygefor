...
{
    var service = 'sygefor_list.batch.csv.' + $dialogParams.service;
        ...
        $http(
            ...
            function (data) { //response should contain the file url
                if (data.fileUrl) {
                    ...
                    var url = Routing.generate('sygefor_list.batch_operation.get_file', {service: service, file: data.fileUrl });
                    // changin location :
                    $window.location = url;
                }
            });
