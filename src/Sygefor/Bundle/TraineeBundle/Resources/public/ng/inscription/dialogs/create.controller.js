    ...
    $scope.getTraineeList = function (pref) {
        ...
        return $http.post(url, {
            ...
            angular.forEach (res.data.items, function (item) {
                adresses.push ({label: item.fullName, value:item.id, organization: (item.organization.name) ? item.organization.name : '', institution: (item.institution.name) ? item.institution.name : ''})
            });
