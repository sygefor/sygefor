...
{
    $scope.service = 'sygefor_list.batch.inscription_status_change';
    ...
    $scope.ok = function () {
        ...
        var data = {
            options: {
                ...
                targetClass: 'SygeforTraineeBundle:Inscription',
        ...
        $http(
            ...
            function (data) { //no response expected...
                $scope.dialog.close();
            }
        );
    ...
    $scope.previewAttachment = function (attachmentTemplate) {
        ...
        var url = Routing.generate('sygefor_list.batch_operation.execute', {id: 'sygefor_list.batch.publipost.inscription'});
        ...
        $http(
            ...
            function (data) { //response should contain the file url
                if (data.fileUrl) {
                    var url = Routing.generate('sygefor_list.batch_operation.get_file', {service: 'sygefor_list.batch.publipost.inscription', file: data.fileUrl, filename: attachmentTemplate.fileName,pdf: true});
                    // changin location :
                    $window.location = url;
                }
            });
